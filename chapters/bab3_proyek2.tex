...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.6\textwidth]{gambar/media/diagram-scrum.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{gambar/media/Usecasenew.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=0.9\textwidth]{gambar/media/erd.png}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=1.0\textwidth]{gambar/media/class-diagram.png}
    \caption{Class Diagram}
    \label{fig:class_diagram}
\end{figure}

Berdasarkan Class Diagram pada Gambar \ref{fig:class_diagram}, Sistem Informasi Peminjaman Sarana dan Prasarana Kampus dirancang menggunakan pendekatan berorientasi objek dengan pemisahan tanggung jawab yang jelas antar kelas. Perancangan ini mencakup kelas entitas utama, kelas pendukung, serta kelas layanan dan repositori untuk mendukung pengelolaan data, penerapan logika bisnis, dan akses basis data secara terstruktur.

Kelas \textbf{User} merepresentasikan aktor dalam sistem, yaitu Mahasiswa, Sarpras, Security, dan Admin, yang dibedakan melalui atribut \texttt{RoleEnum}. Setiap pengguna terasosiasi dengan satu \textbf{Organisasi}, sehingga seluruh aktivitas peminjaman terikat pada unit atau organisasi tertentu. Proses bisnis utama dimodelkan dalam kelas \textbf{Peminjaman}, yang memiliki relasi \textit{many-to-one} dengan \textbf{User}, sehingga satu pengguna dapat melakukan lebih dari satu transaksi peminjaman. Setiap transaksi peminjaman terhubung dengan satu \textbf{Ruangan} dan satu \textbf{Kegiatan} sebagai konteks penggunaan fasilitas. Relasi antara peminjaman dan \textbf{Barang} bersifat \textit{many-to-many}, sehingga digunakan kelas perantara \textbf{PeminjamanBarangDetail} untuk mencatat detail barang yang dipinjam pada setiap transaksi. Pengelolaan status peminjaman dilakukan melalui \texttt{PeminjamanStatusEnum} untuk memastikan alur proses peminjaman berjalan secara sistematis.

Aspek verifikasi lapangan dimodelkan melalui kelas \textbf{KehadiranPeminjam} yang memiliki relasi \textit{one-to-one} dengan kelas \textbf{Peminjaman} dan berfungsi mencatat status serta waktu verifikasi kehadiran. Selain itu, sistem dilengkapi dengan kelas \textbf{Mailbox} sebagai media penyampaian pesan atau notifikasi sistem kepada pengguna serta \textbf{LogAktivitas} untuk mencatat jejak aktivitas pengguna sebagai bentuk audit dan akuntabilitas sistem.
...
\newpage
Pada lapisan layanan, AuthService berfungsi menangani proses autentikasi pengguna, sedangkan PeminjamanService mengelola logika bisnis peminjaman secara terstruktur. Akses dan pengelolaan data pada basis data difasilitasi oleh UserRepository dan PeminjamanRepository sebagai penghubung antara lapisan layanan dan penyimpanan data. Uraian fungsi dan peran setiap kelas dalam sistem disajikan secara ringkas pada Tabel \ref{tab:deskripsi_kelas} sebagai acuan pemahaman struktur kelas pada Class Diagram. Dengan struktur kelas dan relasi yang jelas, Class Diagram ini menjadi pedoman utama dalam proses implementasi sistem.
